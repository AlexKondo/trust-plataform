...
**Hard dependencies:** IP-001, IP-008, IP-009, IP-014

## 1. Objective

Build minimum operational console/APIs for verification queues already present plus payment exceptions, disputes, refunds, risk flags, evidence review, user support lookup, configuration visibility and audit exploration. Enforce privileged roles and reason capture.
...
No enterprise tenant control plane; no unrestricted database admin UI; no silent data edits.
...
Admin actions authorized/audited; support can trace an order/payment end-to-end; sensitive data masked appropriately; manual resolution cannot bypass ledger/payment invariants.
...
- `IP-018-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-018 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-018-COMPLETION-REPORT.md.
```
