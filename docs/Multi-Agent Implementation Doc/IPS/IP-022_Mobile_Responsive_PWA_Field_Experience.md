...
**Hard dependencies:** IP-016, IP-017

## 1. Objective

Harden mobile-first experience and optionally PWA installability/offline-safe shell for field use. Optimize Partner execution actions, Member tracking/approval, camera/file evidence, network retry and deep links. Preserve web codebase unless native app is explicitly justified.
...
No native iOS/Android rewrite; no invasive background GPS.
...
Core journeys work at common mobile widths; transient network failure safe; PWA metadata if implemented; camera/file flows usable; accessibility and performance checks.
...
- `IP-022-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-022 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-022-COMPLETION-REPORT.md.
```
