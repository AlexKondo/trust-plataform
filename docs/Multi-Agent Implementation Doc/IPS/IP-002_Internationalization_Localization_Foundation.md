...
**Primary owner:** Experience/Foundation  
**Hard dependencies:** IP-000

## 1. Objective

Create/complete locale architecture with PT-BR default and per-user preference; externalize user-facing strings; locale-aware dates/times/currency/numbers; fallback strategy; API/user profile locale field if absent; translation catalog structure; frontend language selector; ensure emails/notifications can resolve locale.
...
Do not translate internal identifiers; do not implement multi-region; do not require all world languages.
...
PT-BR default works; at least one secondary test locale proves architecture; no critical user-facing strings hard-coded in new flows; locale preference persists; formatting tests pass.
...
- `IP-002-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-002 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-002-COMPLETION-REPORT.md.
```
