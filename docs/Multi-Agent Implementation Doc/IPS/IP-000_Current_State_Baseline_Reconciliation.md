...
## IP-000 — Current State Baseline & Reconciliation

**Primary owner:** Architecture/Integrator  
**Hard dependencies:** none

## 1. Objective

Freeze the exact `origin/main` SHA as `TRUST MULTI-AGENT BASELINE`; inventory modules/routes/screens/migrations/events/tests; verify PACK-00..03 closure in code; classify every IP as IMPLEMENT/PARTIAL/VERIFY_ONLY/DEFERRED/BLOCKED_EXTERNAL; verify migration 0027 and `change-order-evidences` bucket deployment status; map shared-file collision hotspots; record baseline test/lint/typecheck results.
...
No feature implementation. Do not apply shared/prod migrations. Do not reinterpret product scope.
...
Baseline SHA recorded; capability matrix complete; dependency classifications complete; known debt recorded; baseline regression executable; no ambiguity about what agents may rebuild.
...
- `IP-000-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-000 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-000-COMPLETION-REPORT.md.
```
