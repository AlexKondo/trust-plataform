...
**Primary owner:** Payments/Marketplace  
**Hard dependencies:** IP-007

## 1. Objective

Complete cancellation and dispute policy engine around order/payment state. Support dispute opening, evidence, response, admin/manual resolution foundation, full/partial refund domain flows, cancellation before/after execution with explicit financial consequences, and idempotent refund accounting. Rules should be data/configurable where practical.
...
No AI adjudication; no invented legal policy; if fee/refund business percentages are undecided, implement configurable policy with safe defaults only when existing approved behavior supports them, otherwise escalate.
...
Every lifecycle state has deterministic allowed cancellation/dispute/refund actions; no double refund; refund never exceeds funded amount; evidence/audit complete; Member/Partner permissions tested.
...
- `IP-008-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-008 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-008-COMPLETION-REPORT.md.
```
