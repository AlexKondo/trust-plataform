...
**Hard dependencies:** IP-003, IP-004, IP-015

## 1. Objective

Add optional, feature-flagged AI assistance where it improves local-service conversion: help Member structure a service request, summarize issue, suggest clarifying questions, assist Partner quote description, and explain comparison factors. Model provider behind gateway/port; prompts/versioning/audit; deterministic product rules remain outside LLM.
...
No autonomous purchase/acceptance; no AI Trust Score; no AI dispute verdict; no agent marketplace/runtime platform; no sensitive data sent without policy.
...
AI can be disabled with no core-flow breakage; outputs clearly advisory; structured validation; timeout/fallback; prompt-injection/data-leak tests appropriate to scope.
...
- `IP-019-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-019 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-019-COMPLETION-REPORT.md.
```
