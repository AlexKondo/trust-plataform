...
**Hard dependencies:** IP-001, IP-002

## 1. Objective

Implement Release-1 privacy controls: data inventory/classification, consent/legal-version capture where required, access/export request foundation, deletion/anonymization workflow respecting financial/audit retention, location/evidence retention policy, privacy notices hooks, access audit for sensitive data.
...
Do not delete legally/audit-required financial history; no enterprise legal-hold platform; no unsupported legal claims.
...
Sensitive categories mapped; user request workflow exists; retention jobs/policies testable; access controls and audit verified; locale-ready privacy content placeholders/config.
...
- `IP-021-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-021 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-021-COMPLETION-REPORT.md.
```
