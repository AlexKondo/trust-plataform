...
**Primary owner:** Security/Trust  
**Hard dependencies:** IP-001, IP-007

## 1. Objective

Implement pragmatic Release-1 safety controls: rate limits, suspicious transaction/change-order patterns, account abuse signals, evidence/upload abuse limits, review abuse, referral abuse hooks, admin risk flags, step-up/MFA hooks for critical operations if baseline supports them, and immutable investigation trail.
...
No surveillance; no invasive phone monitoring; no automated guilt; no black-box blocking without reason/audit; no enterprise SIEM project.
...
High-risk operations have abuse controls; flags are explainable; manual review path exists; rate limits canonical; false-positive-safe behavior; security tests.
...
- `IP-014-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-014 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-014-COMPLETION-REPORT.md.
```
