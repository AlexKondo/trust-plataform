...
Implement/complete Partner availability windows, proposed/confirmed appointment, reschedule controls, service address/location privacy, travel/arrival status and ETA abstraction. GPS may use provider/device adapter; exact map provider must be behind a port. Support Member visibility of relevant arrival progress without leaking continuous precise location beyond need.
...
No fleet product; no invasive background tracking; no assumption of a specific maps vendor if none is configured.
...
Appointment lifecycle is auditable; conflicts handled; reschedule rules deterministic; ETA adapter exists; privacy boundaries tested; notification hooks emitted.
...
- `IP-005-COMPLETION-REPORT.md` generated from template.
...
```text
Implement IP-005 only.
Do not self-approve.
Do not merge to main.
If blocked, create a Conflict Escalation artifact.
After implementation and tests, generate IP-005-COMPLETION-REPORT.md.
```
