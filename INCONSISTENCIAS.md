...
| 13 | TRS-015 exibe transações/reviews do MRK, mas nenhum evento MRK está mapeado como Trust Event | **RESOLVIDO (Módulos 8 e 9)**: `MarketplaceOrder.CustomerConfirmed` +40 (prestador), `MarketplaceOrder.Cancelled` −20 (quem cancelou), `MarketplaceReview.Created` +30/+5/−30 por faixa de nota (avaliado), `MarketplaceDispute.Resolved` −60/−30 quando a decisão reconhece culpa. `MarketplaceDispute.Opened` **não** pontua: abrir disputa não é prova de culpa. Regras seed em `0018_marketplace_trust_rules_seed.sql` e `0020_marketplace_review_rules_seed.sql`. |
...
| 32 | MRK-013 cria o pedido (BR-006) e MRK-015 BR-007 exige a mesma transação, mas o pedido é feature do módulo seguinte | O **Módulo 7 cria `marketplace_orders`** e o pedido em `CREATED` junto do aceite (é a única forma de honrar a atomicidade). O Módulo 8 estende a tabela e implementa a máquina de 13 estados. Não existe endpoint de criação manual de pedido. |
| 33 | MRK-009 BR-007 exige que proposta vencida não seja aceita, mas nenhuma spec define quem faz a varredura de expiração | MVP sem job: o status **EXPIRED é derivado** de `expires_at` — a leitura apresenta EXPIRED e nenhuma operação (aceitar/rejeitar/atualizar/contrapor) passa. A coluna `status` só muda quando alguém age. Se a varredura entrar, será um job pg-boss publicando `MarketplaceOffer.Expired`. |
| 34 | MRK-009 §6.3/§6.5 pedem "controlar ofertas pendentes" e uma `MarketplaceOfferAlreadyExistsException`, mas nenhuma BR diz quantas propostas podem coexistir | **Uma proposta viva por negociação**: com uma PENDING não vencida em aberto, criar outra responde 409. A próxima rodada passa por atualizar (MRK-010), retirar (MRK-011) ou decidir (MRK-012/013/014) — é o que dá sentido à cadeia `parentOfferId`. |
| 35 | MRK-020 cria `marketplace_order_checkins` e MRK-021 cria `marketplace_order_execution_events` com tipo CHECK_IN — duas tabelas para o mesmo fato | **Uma tabela só**: `marketplace_order_execution_events` com `event_type` CHECK_IN/CHECK_OUT (o modelo do MRK-021 é superconjunto do MRK-020 e já lista CHECK_IN como tipo). |
